...
.r-btns .rbtn { display:none; }
#btn-share-view { display:inline-flex !important; }
</style>
</head>
<body>
...
async function exportPDF(){const btn=document.getElementById('btn-pdf');const orig=btn.textContent;btn.textContent='Gerando PDF...';btn.disabled=true;try{const{jsPDF}=window.jspdf;const el=document.getElementById('report-page');const canvas=await html2canvas(el,{backgroundColor:'#F3F4F6',scale:1.5,useCORS:true,logging:false});const imgData=canvas.toDataURL('image/jpeg',0.92);const pdf=new jsPDF({orientation:'p',unit:'mm',format:'a4'});const pw=pdf.internal.pageSize.getWidth(),ph=pdf.internal.pageSize.getHeight();const iw=pw,ih=(canvas.height*iw)/canvas.width;let y=0;while(y<ih){if(y>0)pdf.addPage();pdf.addImage(imgData,'JPEG',0,-y,iw,ih);y+=ph;}pdf.save(`relatorio-diagnostico-${new Date().toLocaleDateString('pt-BR').replace(/\//g,'-')}.pdf`);}catch(e){alert('Erro ao gerar PDF: '+e.message);}finally{btn.textContent=orig;btn.disabled=false;}}
...
<script>
// Auto-render report from server data
window.addEventListener('DOMContentLoaded', function() {
  const data = {report_data};
  S.results = data.results || [];
  S.mainTags = data.mainTags || [];
  S.themeTags = data.themeTags || [];
  S.mainCounts = data.mainCounts || {};
  S.themeCounts = data.themeCounts || {};
  S.total = data.total || 0;
  S.screenshotDataUrl = data.screenshotDataUrl || null;
  S.analysisData = data.analysisData || null;
  buildReport(
    S.results, S.mainTags, S.themeTags,
    S.mainCounts, S.themeCounts, S.total,
    data.analysisData, '', '', '', ''
  );
  // Override post info from saved HTML
  if(data.postTitle) document.getElementById('r-post-title').innerText = data.postTitle;
  if(data.postMeta) document.getElementById('r-post-meta').innerHTML = data.postMeta;
  if(data.postStats) document.getElementById('r-post-stats').innerHTML = data.postStats;
  if(data.postBadge) document.getElementById('r-post-badge').innerText = data.postBadge;
});
</script>
</body>
</html>